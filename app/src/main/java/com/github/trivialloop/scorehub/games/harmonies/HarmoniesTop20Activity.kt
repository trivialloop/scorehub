...
import com.github.trivialloop.scorehub.databinding.ActivityHarmoniesTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class HarmoniesTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityHarmoniesTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityHarmoniesTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            // Harmonies: highest score = best -> DESC order (default getTop20ByGameType)
            ...
            val entries = results.map { result ->
                ...
                HarmoniesTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = HarmoniesTop20Adapter(entries)
        }
    }
...
data class HarmoniesTop20Entry(val result: GameResult, val color: Int)

class HarmoniesTop20Adapter(private val entries: List<HarmoniesTop20Entry>) :
    RecyclerView.Adapter<HarmoniesTop20Adapter.ViewHolder>() {
