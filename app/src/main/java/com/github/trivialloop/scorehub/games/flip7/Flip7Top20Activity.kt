...
import com.github.trivialloop.scorehub.databinding.ActivityFlip7Top20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class Flip7Top20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityFlip7Top20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityFlip7Top20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            val entries = database.gameResultDao().getTop20ByGameType(GAME_TYPE).map { result ->
                ...
                Flip7Top20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = Flip7Top20Adapter(entries)
        }
    }
...
data class Flip7Top20Entry(val result: GameResult, val color: Int)

class Flip7Top20Adapter(private val entries: List<Flip7Top20Entry>) :
    RecyclerView.Adapter<Flip7Top20Adapter.ViewHolder>() {
