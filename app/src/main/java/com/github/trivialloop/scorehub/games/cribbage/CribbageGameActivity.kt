...
class CribbageGameActivity : AppCompatActivity() {
    ...
    private fun buildRoundRow(round: CribbageRound, roundIndex: Int): LinearLayout {
        ...
        listOf(p0, p1).forEach { player ->
            ...
            // Cribbage: higher in-play = better
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            android.R.id.home -> {
                AlertDialog.Builder(this).setTitle(R.string.cribbage_quit_game).setMessage(R.string.cribbage_quit_game_message)
