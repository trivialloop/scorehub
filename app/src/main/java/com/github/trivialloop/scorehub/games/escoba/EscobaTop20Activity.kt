...
import com.github.trivialloop.scorehub.databinding.ActivityEscobaTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class EscobaTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityEscobaTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityEscobaTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            // Escoba: highest score = best → order DESC (default getTop20ByGameType)
            ...
            val entries = results.map { result ->
                ...
                EscobaTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = EscobaTop20Adapter(entries)
        }
    }
...
data class EscobaTop20Entry(val result: GameResult, val color: Int)

class EscobaTop20Adapter(private val entries: List<EscobaTop20Entry>) :
    RecyclerView.Adapter<EscobaTop20Adapter.ViewHolder>() {
