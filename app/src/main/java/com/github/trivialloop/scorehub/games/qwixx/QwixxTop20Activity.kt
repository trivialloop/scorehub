...
import com.github.trivialloop.scorehub.databinding.ActivityQwixxTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class QwixxTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityQwixxTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityQwixxTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            ...
            val entries = results.map { result ->
                ...
                QwixxTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = QwixxTop20Adapter(entries)
        }
    }
...
data class QwixxTop20Entry(val result: GameResult, val color: Int)

class QwixxTop20Adapter(private val entries: List<QwixxTop20Entry>) :
    RecyclerView.Adapter<QwixxTop20Adapter.ViewHolder>() {
