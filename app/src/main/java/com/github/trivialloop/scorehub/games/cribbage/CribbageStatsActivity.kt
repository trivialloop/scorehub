...
import com.github.trivialloop.scorehub.databinding.ActivityCribbageStatsBinding
    ...
    private fun loadStats() {
        lifecycleScope.launch {
            database.playerDao().getAllPlayers().collect { players ->
                val statsList = mutableListOf<CribbagePlayerStatsEntry>()
                ...
                statsList.sortWith(
                    compareByDescending<CribbagePlayerStatsEntry> { it.winPercentage }
                ...
                binding.recyclerView.adapter = CribbageStatsAdapter(statsList)
            }
        }
    }
...
data class CribbagePlayerStatsEntry(
    val player: Player,
    val totalGames: Int,
...
class CribbageStatsAdapter(private val stats: List<CribbagePlayerStatsEntry>) :
    RecyclerView.Adapter<CribbageStatsAdapter.ViewHolder>() {
