...
import com.github.trivialloop.scorehub.databinding.ActivitySkyjoTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class SkyjoTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivitySkyjoTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySkyjoTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            // In Skyjo, lower score = better, so we fetch and sort ASC
            val results = database.gameResultDao().getTop20ByGameType(GAME_TYPE)
            // getTop20ByGameType orders DESC; for Skyjo we want ASC (lowest score = best)
            ...
            val entries = sorted.map { result ->
                ...
                SkyjoTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }

            binding.recyclerView.adapter = SkyjoTop20Adapter(entries)
        }
    }
...
data class SkyjoTop20Entry(val result: GameResult, val color: Int)

class SkyjoTop20Adapter(private val entries: List<SkyjoTop20Entry>) :
    RecyclerView.Adapter<SkyjoTop20Adapter.ViewHolder>() {
