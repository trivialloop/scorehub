...
import com.github.trivialloop.scorehub.databinding.ActivityLigrettoTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class LigrettoTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityLigrettoTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLigrettoTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            // Ligretto: highest score = best -> DESC order (default getTop20ByGameType)
            val entries = database.gameResultDao().getTop20ByGameType(GAME_TYPE).map { result ->
                ...
                LigrettoTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = LigrettoTop20Adapter(entries)
        }
    }
...
data class LigrettoTop20Entry(val result: GameResult, val color: Int)

class LigrettoTop20Adapter(private val entries: List<LigrettoTop20Entry>) :
    RecyclerView.Adapter<LigrettoTop20Adapter.ViewHolder>() {
