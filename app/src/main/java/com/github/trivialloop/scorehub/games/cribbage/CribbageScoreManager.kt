 ...
 *     this phase (doing so locks in play for both players).
 ...
 *  round has ANY in play activity (first +/− press in the new round).
 */
data class CribbageRound(
    ...
    val inPlayScores: MutableMap<Long, Int> = mutableMapOf(
        firstPlayerId to 0,
        dealerId      to 0
    )
    ...
    /**
     * in play is editable until the first player enters their end-of-round score.
     */
    fun isInPlayEditable(): Boolean = !isFirstPlayerHandEntered()
