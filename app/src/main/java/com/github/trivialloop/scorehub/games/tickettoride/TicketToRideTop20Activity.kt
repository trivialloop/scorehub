...
import com.github.trivialloop.scorehub.databinding.ActivityTickettorideTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class TicketToRideTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityTickettorideTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityTickettorideTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            // Highest total = best -> DESC order (default getTop20ByGameType)
            val entries = database.gameResultDao().getTop20ByGameType(GAME_TYPE).map { result ->
                ...
                TicketToRideTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = TicketToRideTop20Adapter(entries)
        }
    }
...
data class TicketToRideTop20Entry(val result: GameResult, val color: Int)

class TicketToRideTop20Adapter(private val entries: List<TicketToRideTop20Entry>) :
    RecyclerView.Adapter<TicketToRideTop20Adapter.ViewHolder>() {
