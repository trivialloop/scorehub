...
import com.github.trivialloop.scorehub.databinding.ActivityAkropolisTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class AkropolisTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityAkropolisTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAkropolisTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            val entries = database.gameResultDao().getTop20ByGameType(GAME_TYPE).map { result ->
                ...
                AkropolisTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = AkropolisTop20Adapter(entries)
        }
    }
...
data class AkropolisTop20Entry(val result: GameResult, val color: Int)

class AkropolisTop20Adapter(private val entries: List<AkropolisTop20Entry>) :
    RecyclerView.Adapter<AkropolisTop20Adapter.ViewHolder>() {
