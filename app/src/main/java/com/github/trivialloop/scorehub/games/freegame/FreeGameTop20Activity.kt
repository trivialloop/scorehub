...
import com.github.trivialloop.scorehub.databinding.ActivityFreegameTop20Binding
import com.github.trivialloop.scorehub.utils.LocaleHelper
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.*

class FreeGameTop20Activity : AppCompatActivity() {

    private lateinit var binding: ActivityFreegameTop20Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityFreegameTop20Binding.inflate(layoutInflater)
    ...
    private fun loadTop20() {
        lifecycleScope.launch {
            val entries = database.gameResultDao().getTop20ByGameType(GAME_TYPE).map { result ->
                ...
                FreeGameTop20Entry(result, player?.color ?: android.graphics.Color.GRAY)
            }
            binding.recyclerView.adapter = FreeGameTop20Adapter(entries)
        }
    }
...
data class FreeGameTop20Entry(val result: GameResult, val color: Int)

class FreeGameTop20Adapter(private val entries: List<FreeGameTop20Entry>) :
    RecyclerView.Adapter<FreeGameTop20Adapter.ViewHolder>() {
